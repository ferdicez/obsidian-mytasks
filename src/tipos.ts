...
// Nome exibido de um dos três campos de comportamento na captura. Vem da CHAVE configurada em
// Configurações → Geral → Nomenclatura → "Chaves técnicas do frontmatter", que é onde ela já renomeia
// esses campos — não existe (nem deve existir) um segundo lugar pra dar nome a eles. A chave é
// `snake_case` por ser nome de propriedade YAML; aqui ela vira texto legível ("manter_historico" →
// "manter historico"), do mesmo jeito que as propriedades customizadas mostram o rótulo delas.
export function rotuloFixo(config: ConfigEfetivaGrupo, campo: keyof ChavesFixas): string {
	const chave = config.chavesFixas?.[campo]?.trim() || CHAVES_FIXAS_PADRAO[campo];
